...
import { ModulesData } from 'src/modules/qbuilder/_components/blocksPanel/interface';
...
export default {
  ...
  //Delete block
  deleteblock(blockId, params = {}): Promise<any> {
    return new Promise((resolve, reject) => {
      baseService.delete('apiRoutes.qbuilder.blocks', blockId, params).then(response => {
        resolve(true);
      }).catch(error => reject(error));
    });
  },
